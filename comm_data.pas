...
const
  ...
  PM_PLUGIN_RESTRICT_ADDED	    = 1106; //commfort -> plugin: Новое ограничение (бан):  число(ID ограничения) + срок(оставшийся до истечения ограничения) + число(тип идентификации) + текст(учетная запись) + текст(IP-адрес) + текст(диапазон IP-адресов) + текст(ID компьютера) + число(тип ограничения) + текст(канал) + текст(учетная запись модератора) + текст(причина)
  PM_PLUGIN_RESTRICT_DELETED    = 1107; //commfort -> plugin: Снятие ограничения (бана): текст(учетная запись модератора, снявшего ограничение) + число(ID ограничения) + число(тип идентификации) + текст(учетная запись) + текст(IP-адрес) + текст(диапазон IP-адресов) + текст(ID компьютера) + число(тип ограничения) + текст(канал) + текст(учетная запись модератора, добавившего ограничение) + текст(причина)
  ...
  GD_USERINFO_GET               = 1063; //plugin -> commfort: Информация о пользователе. Блок данных (исходящий): текст(имя пользователя). Блок данных (входящий): текст(IP-адрес) + текст(публично отображаемый IP-адрес) + текст(ID компьютера) + число(иконка пользователя) + текст(состояние) + текст(версия клиента) + число(состояние активности окна программы) + число(время простоя в секундах) + текст(активный процесс)
  GD_RIGHT_GET                  = 1090; //plugin -> commfort: Права учетной записи. Блок данных (исходящий): текст(учетная запись) + число(вид права) + текст(канал). Блок данных (входящий): число(тип активности права)
  ...
  {$ELSE}
  PLU_NAME = 'Линковщик серверов '+PLU_VER+' (клиент)';
  {$ENDIF}
