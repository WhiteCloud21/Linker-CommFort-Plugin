...
begin
  ...
  begin
    len:=Length(CompID);
    CopyMemory(@msg[i], @len, 4);
    CopyMemory(@msg[i+4], @CompID[1], len*2);
    i:=i+len*2+4;
  end;
...
var
  ...
  i, len: DWord;
begin
  len:=Length(Name)*2+Length(User)*2+8;
  SetLength(msg, len);
  len:=Length(Name);
  CopyMemory(@msg[0], @len, 4);
  CopyMemory(@msg[4], @Name[1], len*2);
  ...
  CopyMemory(@msg[i+4], @User[1], len*2);
  i:=i+len*2+4;
  //CommfortProcess(dwPluginID, PM_PLUGIN_ACCOUNT_AGREE, @msg[0], i);
  MsgQueue.InsertMsg(PM_PLUGIN_ACCOUNT_AGREE, msg, i);
end;
...
begin
  ...
  CopyMemory(@msg[i+4], @User[1], len*2);
  i:=i+len*2+4;
  //CommfortProcess(dwPluginID, PM_PLUGIN_ACCOUNT_DEL, @msg[0], i);
  MsgQueue.InsertMsg(PM_PLUGIN_ACCOUNT_DEL, msg, i);
end;
...
var
  ...
  i, len: DWord;
begin
  len:=Length(Name)*2+Length(User)*2+Length(Password)*2+16;
  SetLength(msg, len);
  len:=Length(Name);
  CopyMemory(@msg[0], @len, 4);
  CopyMemory(@msg[4], @Name[1], len*2);
  ...
  len:=Length(Password);
  CopyMemory(@msg[i], @len, 4);
  CopyMemory(@msg[i+4], @Password[1], len*2);
  i:=i+len*2+4;
  //CommfortProcess(dwPluginID, PM_PLUGIN_PASSWORD_CHANGE, @msg[0], i);
  MsgQueue.InsertMsg(PM_PLUGIN_PASSWORD_CHANGE, msg, i);
end;
